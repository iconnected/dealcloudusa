<?
	...
	*/
	...
	if (mysql_num_rows($result_slider) < TOTAL_SLIDER_ITEMS && $slider_feature == "ON"){
		$tempArr = array();
		while ($row1 = mysql_fetch_assoc($result_slider)) {
				array_push($tempArr,$row1['slide_order']);
		}
		if($tempArr) $whereslide =  " AND slide_order NOT IN(".implode(',',$tempArr).")";
		...
		$sql = "(".$sql.")" ." UNION (SELECT * FROM Slider WHERE image_id > 0 ".$where_clause." $whereslide  ) ORDER BY slide_order   LIMIT ".TOTAL_SLIDER_ITEMS;		
	...
	if (mysql_num_rows($result_slider) > 0 && $slider_feature == "ON"){
		...
		*/
		//echo mysql_num_rows($result_slider);
